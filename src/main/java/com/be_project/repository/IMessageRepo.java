...
public interface IMessageRepo extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT COUNT(m) FROM Message m WHERE m.receiver.id = :receiverId AND m.status = 'Chưa xem'")
    long countUnreadMessagesByReceiverId(@Param("receiverId") long receiverId);

    @Query("SELECT COUNT(m) FROM Message m WHERE m.sender.id = :senderId AND m.receiver.id = :accountLoginId AND m.status = 'Chưa xem'")
